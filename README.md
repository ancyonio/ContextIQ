...
[![Docs](https://img.shields.io/badge/docs-genaivikky.github.io%2FContextIQ-3c8772)](https://genaivikky.github.io/ContextIQ/)

📖 **[Read the docs →](https://genaivikky.github.io/ContextIQ/)** &nbsp;·&nbsp; [Quick start](guide/quick-start.md) &nbsp;·&nbsp; [CLI reference](guide/cli.md) &nbsp;·&nbsp; [When to use what](guide/when-to-use.md)
...
**Read this honestly — and mind the budget.** ContextIQ nearly always finds the
right *file*; whether it carries *every* required symbol and fact into one pack
is strongly budget-dependent, and the 6,000-token headline is the pessimal
corner, not a ceiling:

| Budget | `answerable_rate` | `symbol_recall` |
|--:|--:|--:|
| 6,000 | 0.64 | 0.77 |
| **12,000** | **0.84** | **0.90** |
| 20,000 | 0.85 | 0.90 |

Measured on the deterministic hash backend (the reproducible CI backend, and the
*weaker* one — a warmed neural model lifts the identifier-opaque cases further).
A failure triage of the 6k misses found **zero extraction gaps**: every missing
symbol is indexed, so failures are budget/ranking misses, not things the graph
can't see. Practically: give an answer-critical task **≥12k tokens**, or call
`get_symbol` for anything the pack lists as dropped. Raising the *6k* number
specifically is the main open quality work, but it is a conservative floor, not
the number you get in real use.
